...
// BoolType构造函数 - 只提供基本构造函数，转换方法由标准库提供
...
// CharType构造函数 - 只提供基本构造函数，转换方法由标准库提供
...
IntegerType::IntegerType() : NumericType("int", true) {
    ...
    addUserMethod(new BuiltinFunction([](class Scope* scope) -> Value* {
        if (Integer* intInstance = scope->getThis<Integer>()) {
            return new Integer(intInstance->toInt());
        }
        return nullptr;
    }, "toInt()"));
    ...
    addUserMethod(new BuiltinFunction([](class Scope* scope) -> Value* {
        if (Integer* intInstance = scope->getThis<Integer>()) {
            return new Char(intInstance->toChar());
        }
        return nullptr;
    }, "toChar()"));
}
...
DoubleType::DoubleType() : NumericType("double", true) {
    ...
    addUserMethod(new BuiltinFunction([](class Scope* scope) -> Value* {
        if (Double* doubleInstance = scope->getThis<Double>()) {
            return new Integer(doubleInstance->toInt());
        }
        return nullptr;
    }, "toInt()"));
    ...
    addUserMethod(new BuiltinFunction([](class Scope* scope) -> Value* {
        if (Double* doubleInstance = scope->getThis<Double>()) {
            return new Double(doubleInstance->toDouble());
        }
        return nullptr;
    }, "toDouble()"));
    ...
    addUserMethod(new BuiltinFunction([](class Scope* scope) -> Value* {
        if (Double* doubleInstance = scope->getThis<Double>()) {
            return new Char(doubleInstance->toChar());
        }
        return nullptr;
    }, "toChar()"));
}
