...
protected:
    ...
    std::vector<FunctionInfo> getFunctions() override {
        ...
        // 添加IO函数
        functions.emplace_back("open", "open(filename, mode)", 
            std::vector<std::string>{"filename", "mode"},
            [](Scope* scope) -> Value* {
                return builtin_open(scope);
            });
            
        functions.emplace_back("close", "close(handle)", 
            std::vector<std::string>{"handle"},
            [](Scope* scope) -> Value* {
                return builtin_close(scope);
            });
            
        functions.emplace_back("read", "read(handle, length)", 
            std::vector<std::string>{"handle", "length"},
            [](Scope* scope) -> Value* {
                return builtin_read(scope);
            });
            
        functions.emplace_back("write", "write(handle, content, newline)", 
            std::vector<std::string>{"handle", "content", "newline"},
            [](Scope* scope) -> Value* {
                return builtin_write(scope);
            });
